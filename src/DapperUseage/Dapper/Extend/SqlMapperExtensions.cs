...
namespace Dapper
{
    ...
    public static class SqlMapperExtensions
    {
        ...
        /// <returns></returns>
        public static int GetCount<T>(this IDbConnection connection, string whereStr = null, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            var type = typeof(T);
            string sql;
            var name = GetTableName(type);

            sql = "select count(*) from " + name + " where 1=1 " + whereStr;
            return connection.ExecuteScalar<int>(sql, null, transaction: transaction, commandTimeout: commandTimeout);
        }
